...
from TO.parameterizations.mmc import Capsules, MMCCenterpointsConfig, MMCDeformerPipeline, LinearHeightInterpolation, CubicBezierDeformer
...
def constructor_normalization_scale(n_components: int) -> ProblemInstance:
    ...
    parameterization.normalization_scale[[4,5]] = min(rxmin, rymin)
    parameterization.deformer.deformers[1].rnorm = min(rxmin, rymin)
