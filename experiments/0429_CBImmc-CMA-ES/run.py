...
from TO.parameterizations.mmc import Capsules, MMCCenterpointsConfig, MMCDeformerPipeline, CubicBezierDeformer, Infill

def construct_problem(n_components: int) -> ProblemInstance :
    topology = Topology(True, (100,50), 1.0)
    deformer = MMCDeformerPipeline([CubicBezierDeformer(1000), Infill()])
    parameterization = Capsules(topology, False, True, n_components, MMCCenterpointsConfig, deformer, 1_000)
