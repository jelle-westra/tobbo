...
def check_package_status(notebook: bool=True) :
    ...
    if (notebook):
        ...
        if (file_changes or untracked_files) :
            print(f'[NO GO] TO @ {commit.stdout}')
            print('Changes found in the ./TO package, first commit changes before starting experiments.')
            return False
        else:
            print(f'[GO] TO @ {commit.stdout}')
            print('No changes found in the ./TO packge.')
    with open(f'{commit.stdout}.version', 'w') as _ : ...
    return True          
